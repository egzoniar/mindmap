...
const useMindmapStore = create<MindmapStore>((set, get) => ({
  ...
  setThemeColorGroup: (colorGroup: string) => set((state) => {
  ...
  setNodes: (nodes: D_Node[]) => set((state) => ({
    mindmap: {
      ...state.mindmap,
      nodes,
    },
  ...
  addNode: (payload: AddNodePayload) => set((state) => {
    ...
    const newNode: D_Node = {
      ...
      data: { 
        title: "New Node",
        description: "What is the node about?",
  ...
  editNode: (data: NodeData) => set((state) => {
  ...
  removeNode: () => set((state) => {
    ...
    const mindmap = {
      ...state.mindmap,
      nodes: state.mindmap.nodes.filter((node) => node.id !== selectedNode?.id),
  ...
  onNodeSelectionChange: (selectedNode: Node[]) => set((state) => ({
    mindmap: {
      ...state.mindmap,
      selectedNode: selectedNode.length === 0 ? undefined : selectedNode[0],
    }
  })),
  onEdgeSelectionChange: (selectedEdge: Edge[]) => set((state) => ({
    mindmap: {
      ...state.mindmap,
