...
const NodeLayoutTypes: FC<NodeLayoutTypesProps> = ({ type = "Default" }) => {
  ...
  const selectedStyle = cn(
    "border-transparent shadow-md ring-2 ring-green-600"
  );
  const hoverStyle = cn(
    "hover:cursor-pointer hover:border hover:border-gray-400 hover:shadow-md"
  );
    ...
    >
      ...
      {selected && (
        <CircleCheck
          ...
          strokeWidth={2}
          color="green"
          className="absolute z-50 top-[-11px] right-[-10px] bg-white rounded-full"
        />
      )}
    </div>
  );
